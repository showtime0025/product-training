 ...
 * Design Tokens
 * 
 * This file contains all design system tokens used throughout the application.
...
export type BoxShadowKey = keyof typeof tokens.boxShadow;
export type ZIndexKey = keyof typeof tokens.zIndex;
export type BreakpointKey = keyof typeof tokens.breakpoints;

// Utility function to get CSS custom property names
export const getCSSVarName = (path: string): string => {
  return `--${path.replace(/\./g, '-')}`;
};

// Utility function to create CSS custom property values
export const createCSSVars = (obj: any, prefix = ''): Record<string, string> => {
  const vars: Record<string, string> = {};
  
  const traverse = (current: any, currentPrefix: string) => {
    Object.entries(current).forEach(([key, value]) => {
      const varName = getCSSVarName(currentPrefix ? `${currentPrefix}-${key}` : key);
